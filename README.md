...
- The initial Cisco ISE setup was definitely the most challenging part of the project. I first tried using Azure VM Custom Data to automate the initial configuration, but that approach didn't work as expected. I then ran into several issues while configuring ISE through the VM serial console, including getting the initial network and system configuration working correctly. I had to troubleshoot and test different configurations, and work through the errors before I could finally access the ISE web interface.
- ISE refused to save `Local_User_Authentication`, `Guest_Access`, and `Employee_Access` because their rule conditions weren't fully configured. Working through the Library/Editor condition builder made it clear how ISE actually evaluates a rule before applying a result, rather than just accepting whatever's typed in.
- Wasn't that big of an issue, but the B-series wasn't available for the RADIUS client VM in this region, so I compared the available D-series options on CPU, RAM, disk, and cost instead. 
- RADIUS authentication succeeded end to end (`radtest` returned `Access-Accept`), but ISE's Live Logs dashboard showed no entries. Rather than assuming RADIUS itself was broken, I checked the response in detail — `Access-Accept`, correct `User-Name`, and a valid TrustSec SGT in `Cisco-AVPair` — which confirmed authentication was working fine.
